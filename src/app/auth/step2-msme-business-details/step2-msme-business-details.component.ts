...
getSelectedAreaNames(): string {
  ...
  // If all selected
  if (this.formData.selectedAreaIds.length === this.areas.length) {
    return 'Entire City';
  }
...
onSelectAll(event: any) {
  ...
  if (event.target.checked) {
    ...
    // select all areas
    this.formData.selectedAreaIds = this.areas.map(a => a.id);
...
isAllSelected(): boolean {
  ...
  return this.formData.selectedAreaIds?.length === this.areas.length;
